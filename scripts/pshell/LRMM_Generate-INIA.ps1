...
for ( $i=1; $i -lt 24; $i++ ) {
    ...
    $outFile = "\\scotia.sgngroup.net\dfs\shared\Syn4.2.3\TEST AREA\ac00418\LRMM17\settings\LRMM_A" + $i + ".ini"
        
    # Assign this ini file to db for record-keeping
    $lrmmNetworks | where { $_.LRMMRun -eq $i } | foreach { $_.A_ini = $outFile }
